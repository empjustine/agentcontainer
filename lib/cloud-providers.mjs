 ...
 * (`generate-models.json.mjs` `baseUrl`, `generate-opencode.mjs`
 ...
 * consumer's decision:
 *   - `coding-agent/generate-pi-coding-agent.mjs` consumes the
 ...
 * reachability probe, for the HOST (and, in peerProviderUrl, the base
 * path) the deployed llm-reverse-proxy allowlist keys on (docs/d047),
 * and, in gen-lib's case, as the llama-swap peer proxy target; never a
 * peer route — peer routes are resolved at generation time by the probe
 * cascade as `<peerBase>/<host><base-path>` (docs/d047).
...
export const CLOUD_PROVIDERS = Object.freeze({
	...
	google: Object.freeze({
		...
		// openai-completions-only peer routing — docs/d027). NOTE: /v1beta/models
		// itself authenticates via an api-key parameter, not the bearer the
		// generator's probe sends, so google is a catalogOnly row — the
		// models.dev slice is its lineup source, no probes (docs/d033).
 ...
 * full-block layer is owned by generate-pi-coding-agent.mjs's full rows,
 * docs/d022), and so are hyper and inferx (same reasoning, model-016 and
 * model-017). `google` IS
