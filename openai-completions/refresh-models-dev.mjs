 ...
 *
 * Validation is scoped to what THIS folder consumes: the `opencode` and
 * `opencode-go` provider entries generate-peer-cloud.yaml.mjs reads.
 *
...
setLogTool("openai-completions/refresh-models-dev");

const scriptDir = dirname(fileURLToPath(import.meta.url));
const CATALOG_URL = "https://models.dev/api.json";
const OUT = process.argv[2] ?? join(scriptDir, "models.dev.api.json");
const REQUEST_TIMEOUT_MS = 15000;
// Provider entries this folder's generators read from the catalog; each must
// exist with a models map or the payload is rejected.
const REQUIRED_PROVIDERS = ["opencode", "opencode-go"];
