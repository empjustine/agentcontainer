...
// ${qwen36}, …) which llama-swap resolves after merging config.d/.
//
...
import { writeConfigD } from "./gen-lib.mjs";
...
function cacheResolver(m) {
  ...
  // download fallback.  biome-ignore: expanded by llama-swap macros, not JS.
  ...
  return { launcherArgs: `sh ${LAUNCHER} ${repoDir} ${repo} ${m.model} ${m.mmproj || "-"} ${m["model-draft"] || "-"} --` };
}
...
function main() {
  ...
  for (const raw of modelData.models) {
    ...
    if (models[id]) {
      console.warn(`  warning: duplicate derived model id "${id}" (${m["hf-repo"]}) — skipping`);
      continue;
    }
