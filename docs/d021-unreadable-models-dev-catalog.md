...
|----------|-------|----------|
| `openai-completions/gen-lib.mjs` → `loadModelsDev()`, called from `fetchPeerModels()` | `openai-completions/models.dev.api.json` | model ids for the `modelsDev`-flagged peers: `opencode`, `opencode-go` (all models, unfiltered) |
...
```
Error: ENOENT: no such file or directory, open '…/openai-completions/models.dev.api.json'
...
3. **The failure is easy to misread as success** by anything wrapping the
   ...
   but easy to skim past; `openai-completions/generate.sh` now tracks
...
### 1. `openai-completions/gen-lib.mjs` — guard the `modelsDev` branch
...
cd "$W/openai-completions" && mv models.dev.api.json "$W/catalog.hidden"
...
current (see `openai-completions/generate.sh`).
