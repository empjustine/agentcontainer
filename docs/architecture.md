...
- `openai-completions/` is the **multipurpose serving instance**: it owns the
  local-llm generator (`generate-local-llm-models.yaml.js` +
  `llamacpp-model-data.json` + `active-b.json` →
  `config.d/10-local-llm-inference.yaml`, emitted only on GPU-capable
  container hosts), the `generate-general.yaml.js` globals/macros, the peer
  generators (`generate-peer-cloud.yaml.js`, `generate-gfx1030-models.mjs`),
...
`generate-local-llm-models.yaml.js` from `llamacpp-model-data.json`, and only
...
|-------|------|--------------|------|
| `00-general.yaml` | globals + macros | `generate-general.yaml.js` | always |
| `10-local-llm-inference.yaml` + `launch-gguf.sh` | local GGUF `models` | `generate-local-llm-models.yaml.js` | container backend + GPU detected |
| `peer-cloud.yaml` | cloud peers | `generate-peer-cloud.yaml.js` | when any provider answers |
...
|--------|------|--------------------|
| `openai-completions/` | multipurpose serving (local GGUF where capable, peers everywhere) | `gen-lib.mjs`, `generate-general.yaml.js`, `generate-local-llm-models.yaml.js` + `llamacpp-model-data.json` + `active-b.json`, `generate-peer-cloud.yaml.js`, `generate-gfx1030-models.mjs`, `launch-gguf.sh`, `llama-swap-core.json`, `config.d/` + `run.sh`, `build.sh` (termux-native build or image pull) + `run-native.sh` (Termux) |
