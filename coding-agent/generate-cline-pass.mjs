// Emit the pi overlay `model-015-cloud-cline-pass.json`: the ClinePass provider
// (https://docs.cline.bot/getting-started/clinepass) as a single OpenAI-compatible
...
function buildThinkingLevelMap(reasoningOptions) {
  ...
  for (const level of PI_LEVELS) {
    ...
    const effort = Object.keys(EFFORT_TO_PI).find((e) => EFFORT_TO_PI[e] === level);
...
function main() {
  ...
  const out = process.argv[2] ?? join(scriptDir, "model-015-cloud-cline-pass.json");
  const tmp = `${out}.tmp`;
  writeFileSync(tmp, `${JSON.stringify(block, null, 2)}\n`);
  renameSync(tmp, out); // atomic on the same filesystem
  console.warn(`  wrote ${models.length} ClinePass model(s) -> ${out}`);
}
